...
// File contracts/UTA.sol

pragma solidity >=0.6.0 <0.8.4;
contract UTA is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 48000 ether;
    // Initial distribution for the day 2-5 UTA-WETH LP -> UTA pool
    uint256 public constant INITIAL_UTA_POOL_DISTRIBUTION = 672000 ether;
    ...
    /**
     * @notice Constructs the UTA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("UTAToken", "UTA") {
        // Mints 1 UTA to contract creator for initial pool setup
    ...
    function _getUTAPrice() internal view returns (uint256 _utaPrice) {
        try IOracle(utaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("UTA: failed to fetch UTA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _utaPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_utaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setUTAOracle(address _utaOracle) public onlyOperatorOrTaxOffice {
        require(_utaOracle != address(0), "oracle address cannot be 0 address");
        utaOracle = _utaOracle;
    }
    ...
    /**
     * @notice Operator mints UTA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of UTA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentUTAPrice = _getUTAPrice();
            currentTaxRate = _updateTaxRate(currentUTAPrice);
            if (currentUTAPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_utaPool != address(0), "!_utaPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_utaPool, INITIAL_UTA_POOL_DISTRIBUTION);
