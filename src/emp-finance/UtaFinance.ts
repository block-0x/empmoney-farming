import { BUSD_TICKER, ESHARE_TICKER, ETH_TICKER, UTA_TICKER } from './../utils/constants';
...
export class UtaFinance {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WBNB') {
      ...
      } else if (tokenName === 'UTA-BNB-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.UTA, true);
      } else {
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === BNB_TICKER) {
      ...
      switch (tokenName) {
        ...
        case UTA_TICKER: token = this.UTA; break;
        default: token = null;
      }
